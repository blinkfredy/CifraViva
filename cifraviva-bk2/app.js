...
function updateSetlistNavUI() {
  ...
  if(btnNext) btnNext.disabled = activeIdx === -1 || activeIdx >= list.length - 1;
}
...
// Navegación rápida por botones Prev/Next en toolbar
document.getElementById('btnSetlistPrev')?.addEventListener('click', () => {
  const list = getSongsInCurrentSetlist();
  const idx = list.findIndex(s => s.id === currentSongId);
  if(idx > 0) loadSongById(list[idx - 1].id);
});

document.getElementById('btnSetlistNext')?.addEventListener('click', () => {
  const list = getSongsInCurrentSetlist();
  const idx = list.findIndex(s => s.id === currentSongId);
  if(idx !== -1 && idx < list.length - 1) loadSongById(list[idx + 1].id);
});
